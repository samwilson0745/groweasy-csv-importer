...
import { AiClientError, callAi } from "./aiClient";
...
// Retry backoff: exponential (base * 2^attempt), capped, plus a little
// jitter so concurrent batches don't all retry in lockstep. The previous
// fixed 400ms/800ms delays were nowhere near enough for a provider that
// explicitly says "high demand, try again later" (e.g. Gemini 503) - all
// retries burned through in about a second while the provider was still
// overloaded. A ~1.5s/3s spread gives transient overload a real chance to
// clear, while staying well under the route's own timeout budget.
const RETRY_BASE_MS = numFromEnv("AI_RETRY_BASE_MS", 1500, 100, 10000);
const RETRY_MAX_DELAY_MS = numFromEnv("AI_RETRY_MAX_DELAY_MS", 10000, 500, 60000);
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          return { imported, skipped };
                ...
                // Permanent failures (missing/invalid API key, a response that will
          // truncate identically every time) are marked non-retryable at the
          // source - retrying them just delays an inevitable failure, so stop
          // immediately instead of burning through the remaining attempts.
          const isRetryable = !(err instanceof AiClientError) || err.retryable;
                console.error(
                          `[aiExtractor] batch ${batchNumber} attempt ${attempt}/${MAX_RETRIES} failed${
                                      isRetryable ? "" : " (non-retryable)"
                          }:`,
